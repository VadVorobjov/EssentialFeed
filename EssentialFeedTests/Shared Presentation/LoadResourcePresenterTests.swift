...
//  EssentialFeedTests
//
//  Created by Vadims Vorobjovs on 02/10/2022.
//
...
class LoadResourcePresenterTests: XCTestCase {
    ...
    func test_didFinishiLoadingWithMapperError_displaysLocalizedErrorMessageAndStopsLoading() {
        let (sut, view) = makeSUT(mapper: { resource in
            throw anyNSError()
        })
    ...
    func test_didFinishLoadingWithError_displaysLocalizedErroMessageAndStopsLaoding() {
        let (sut, view) = makeSUT()
    ...
    private func localized(_ key: String, file: StaticString = #file, line: UInt = #line) -> String {
        ...
        if value == key {
            XCTFail("Missing localized strign for key: \(key) in table \(table)", file: file, line: line)
        }
