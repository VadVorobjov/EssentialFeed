...
//
//  Created by Vadims Vorobjovs on 24/10/2021.
//
...
class EssentialFeedCacheIntegrationTests: XCTestCase {
	...
	func test_saveFeed_overridesItemsSavedOnASeparateInstance() {
		let feedLoaderToPeformFirstSave = makeFeedLoader()
		let feedLoaderToPerformLastSave = makeFeedLoader()
		let feedLoaderToPerformLoad = makeFeedLoader()
		let firtsFeed = uniqueImageFeed().models
		let latestFeed = uniqueImageFeed().models

		save(firtsFeed, with: feedLoaderToPeformFirstSave)
    ...
    func test_loadImageData_overridersSavedImageDataOnASeparateInstance() {
        let imageLoaderToPerforFirstSave = makeImageLoader()
        ...
        save([image], with: feedLoader)
        save(firstImageData, for: image.url, with: imageLoaderToPerforFirstSave)
    ...
    func test_validateFeedCache_deletesFeedSavedInADistantPass() {
        let feedLoaderToPerformSave = makeFeedLoader(currentData: .distantPast)
        let feedLoaderToPerformValidation = makeFeedLoader(currentData: Date())
        let feed = uniqueImageFeed().models
    ...
    private func makeFeedLoader(currentData: Date = Date(), file: StaticString = #filePath, line: UInt = #line) -> LocalFeedLoader {
    ...
    private func validateCache(with loader: LocalFeedLoader, file: StaticString = #filePath, line: UInt = #line) {
        ...
        loader.validateCache() { result in
            if case let Result.failure(error) = result {
                XCTFail("Expected to validate feed successfulyy, got \(error) instead")
            }
            saveExp.fulfill()
        }
        wait(for: [saveExp], timeout: 1.0)
    ...
    private func expect(_ sut: LocalFeedImageDataLoader, toLoad expectedData: Data, for url: URL, file: StaticString = #filePath, line: UInt = #line) {
        ...
        _ = sut.loadImageData(from: url) { result in
            ...
            case let .failure(error):
                XCTFail("Expected successfull image data result, got \(error) instaed", file: file, line: line)
            }
	...
	private func testSpecificStoreURL() -> URL {
		return cachedDirectory().appendingPathComponent("\(type(of: self)).store")
	}
